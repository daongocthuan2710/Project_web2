<?php
    return [
        ...
        'updateNV' => function($conn,$nv,$id) {
            $query ="UPDATE nhanvien SET TenNV = '".$nv[0]."', Ho_Dem_KH = '".$nv[1]."', SDT= ".$nv[2].", email = '".$nv[3]."', DiaChi = '".$nv[4]."' WHERE IdNV = ".$id;
            $result = mysqli_query($conn,$query);
            if(!$result) {
                return false;
            }
        ...
        'insertNV' => function($conn,$nv) {
            $query ="INSERT INTO sach (TenNV, Ho_Dem_KH,SDT,email,DiaChi) VALUES ('".$nv[0]."','"
            .$nv[1]."',".$nv[2].",'".$nv[3]."','".$nv[4]."')";
            $result = mysqli_query($conn,$query);
            if(!$result) {
                return false;
